...
def detect_text_and_translate(frame, target_language):
    ...
    return extracted_text, detected_language, translated_text

def main():
    cap = cv2.VideoCapture(0)  # Access the device camera
    
    start_time = time.time()  # Record the start time
    duration = 50  # Capture frames for 10 seconds (adjust as needed)
    ...
    while (time.time() - start_time) < duration:
        ...
        text, detected_lang, translated_text = detect_text_and_translate(frame, target_language)
        
        cv2.putText(frame, f"Detected Language: {detected_lang}", (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
        cv2.putText(frame, f"Original Text: {text}", (20, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
        cv2.putText(frame, f"Translated Text ({target_language}): {translated_text}", (20, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2)
        
        cv2.imshow('Text Translation', frame)
        cv2.waitKey(1)  # Wait for 1ms to show the frame
